...
class MatterAttributeSubscribeForm extends ConsumerStatefulWidget {
  final List<MatterCluster> clusters;

  const MatterAttributeSubscribeForm({super.key, required this.clusters});

  @override
  ConsumerState<MatterAttributeSubscribeForm> createState() =>
      _MatterAttributeSubscribeFormState();
}

class _MatterAttributeSubscribeFormState
    extends ConsumerState<MatterAttributeSubscribeForm> {
  final _formKey = GlobalKey<FormState>();
  final _nodeId = TextEditingController();
  final _endpointId = TextEditingController();
  final _minInterval = TextEditingController();
  final _maxInterval = TextEditingController();
  ...
  Widget _field(
    ...
    return SizedBox(
      ...
      child: AppLabeledTextField(
        ...
        label: label,
        keyboardType: label == 'Node ID' ? null : TextInputType.number,
  ...
  void _submit() {
    ...
    ref.read(matterCommandControllerProvider.notifier).subscribeAttribute(
          MatterAttributeSubscribeRequest(
            ...
            minInterval: int.parse(_minInterval.text.trim()),
            maxInterval: int.parse(_maxInterval.text.trim()),
          ),
        );
  }
}
