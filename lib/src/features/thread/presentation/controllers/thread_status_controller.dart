...
final class ThreadStatusController extends StateNotifier<ThreadStatusState> {
  ...
  void _handleMessage(OrchestratorMessage message) {
    ...
    final next = switch (message) {
      ThreadStackStatusReceived(:final running) => current.copyWith(
        stackRunning: running,
      ),
      ThreadInterfaceStatusReceived(:final interfaceUp) => current.copyWith(
        interfaceUp: interfaceUp,
      ),
      ThreadAttachmentStatusReceived(:final attached) => current.copyWith(
        attached: attached,
      ),
      ThreadRoleReceived(:final role) => current.copyWith(role: role),
      ThreadActiveDatasetReceived(:final dataset) => current.copyWith(
      ...
      ThreadMeshcopServiceStatusReceived(:final published) => current.copyWith(
        meshcopPublished: published,
      ),
      _ => current,
    };
